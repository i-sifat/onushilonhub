...
export default function RightFormVerbCombinedPage() {
  ...
  const filteredQuestions = rightFormVerbQuestions.filter(question => {
  ...
  const selectedRule = selectedRuleId ? rightFormVerbRules.find(rule => rule.id === selectedRuleId) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="secondary" className="bg-sf-button/20 text-sf-button">
            {rightFormVerbRules.length} Rules
          </Badge>
        </div>

        {/* Introduction */}
        <div className="bg-sf-bg border border-sf-button/30 rounded-lg p-4">
          <h3 className="text-md font-bold text-sf-text-bold mb-3">
            Right Form of Verb: যা যা লক্ষ্য রাখতে হবে
          </h3>
          <div className="space-y-2 text-sf-text-subtle text-sm">
            <p>• প্রথমেই সম্পূর্ণ Passage-টি পড়ে তা বোঝার চেষ্টা করতে হবে।</p>
            <p>• নিচের Right Form of Verb এর নিয়ম সমূহ প্রয়োগ করতে হবে।</p>
            <p>• Passage-টি কোন Tense-এ লিখা তা বাহির করতে হবে।</p>
          </div>
        </div>
      ...
      <div className="space-y-6">
        ...
        {selectedRule ? (
          <div className="bg-sf-bg border border-sf-button/30 rounded-lg p-6">
            ...
            {/* Notes */}
            {selectedRule.notes && selectedRule.notes.length > 0 && (
              <div>
                <h5 className="text-md font-semibold text-sf-text-bold mb-3">Notes:</h5>
                <div className="space-y-2">
                  {selectedRule.notes.map((note, index) => (
                    <div
                      key={index}
                        ...
                        {question.passage ? (
                          <div>
                            ...
                            {question.blanks && question.blanks.length > 0 && (
                              <div>
                                <p className="font-medium mb-2">Sample Answers:</p>
                                <div className="grid grid-cols-2 gap-1">
                                  {question.blanks.slice(0, 4).map((blank) => (
                                    <div key={blank.id} className="text-xs">
                                      <span className="font-medium">({blank.id})</span> {blank.verb} → {blank.answer}
                                    </div>
                                  ))}
                                  {question.blanks.length > 4 && (
                                    <div className="text-xs text-sf-text-muted col-span-2">
                                      ... and {question.blanks.length - 4} more
                                    </div>
                                  )}
                                </div>
                              </div>
