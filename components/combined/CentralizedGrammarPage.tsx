...
interface Question {
  ...
  question: string;
  ruleId?: number;
  [key: string]: any; // Allow for additional properties specific to different question types
}
          ...
          {question.blanks && question.blanks.length > 0 && (
            <div>
              ...
              <div className="grid md:grid-cols-2 gap-2">
                {question.blanks.slice(0, 6).map((blank: any) => (
                  <div key={blank.id} className="text-sm text-sf-text-subtle">
                    <span className="font-medium">({blank.id})</span> {blank.instruction || blank.answer || blank.verb}
                  </div>
                ))}
                {question.blanks.length > 6 && (
                  <div className="text-sm text-sf-text-muted col-span-2">
                    ... and {question.blanks.length - 6} more items
                  </div>
                )}
              </div>
            </div>
