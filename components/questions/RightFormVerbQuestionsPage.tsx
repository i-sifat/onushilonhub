...
export default function RightFormVerbQuestionsPage() {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedBoard, setSelectedBoard] = useState('All Boards');
  const [selectedYear, setSelectedYear] = useState('All Years');

  const filteredQuestions = useMemo(() => {
    return rightFormVerbQuestions.filter(question => {
                    ...
                    {question.passage ? (
                      <div>
                        ...
                        {question.blanks && question.blanks.length > 0 && (
                          <div className="mt-4">
                            <h4 className="text-md font-semibold text-sf-text-bold mb-2">Verbs to use:</h4>
                            <div className="grid md:grid-cols-2 gap-2">
                              ...
                              {question.blanks.length > 6 && (
                                <div className="text-sm text-sf-text-muted col-span-2">
                                  ... and {question.blanks.length - 6} more verbs
                                </div>
                              )}
                            </div>
                          </div>
